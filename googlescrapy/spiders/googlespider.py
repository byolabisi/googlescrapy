...
from googlescrapy.items import googlescrapyItem
...
"""
class googlescrapySpider(BaseSpider):
    name = 'googlescrapy'
        ...
        for sel in hxs.select('//div[@id="ires"]//li[@class="g"]//h3[@class="r"]'):
            ...
            if len(url):
                if self.download_html:
                    ...
                    yield googlescrapyItem(url=url, name=name, query=response.meta['query'], crawled=datetime.datetime.utcnow().isoformat())
    ...
    def parse_item(self, response):
        ...
        yield googlescrapyItem({'name': name,
                                'url': url,
                                'html': html,
                                'region': self.region,
                                'query': query,
